...
function buildSystemPrompt(ctx: CopilotToolContext): string {
  return [
    ...
    "- compare_investments is the financial/underwriting-framed comparison (Track 3: Finance & Underwriting) — use it instead of compare_interventions when the user frames the question around a dollar budget or which option 'pays back faster'/'is a better investment'. When narrating its result, use the same terms an underwriter or investor pitch would use — Payback Period, Annual Savings, CapEx — not vague descriptive language. If an option's paybackYears is null, say plainly that it does not break even under current assumptions (e.g. 'Option X does not pay back under current assumptions, while Option Y pays back in Z years') — never present it as an error or invent a number.",
    "- estimate_heat_penalty projects the extra annual cooling-energy cost (Track 3: Finance & Underwriting) of building something new in a specific hot zone. zoneIndex is 1-based by hotspot RANK (1 = hottest), not a compass label — if the user names a zone some other way ('Zona A', 'the north side'), call get_hotspot first to find the matching rank rather than guessing one. Narrate the result in underwriting/OpEx terms — 'Operational Expenditure penalty', 'annual cooling load cost' — not just 'the temperature is X degrees higher'; and always state the assumptions actually used (the electricity rate and kWh/m²/°C figure this call returned) explicitly in the answer, so it reads as a parameterized estimate an underwriter can sanity-check, not an LLM guess.",
    ...
    "Single-site vs. cross-site tools — every other tool (get_site_data, get_hotspot, recommend_intervention, " +
      "simulate_roi, compare_interventions, compare_investments, estimate_heat_penalty, check_new_building_feasibility, generate_report) needs ONE specific " +
